...
export default {
    data() {
        return {
            ...
            userRuleValidate: {
                ...
                start_time: [{ required: true, type: 'date', message: '加班日期不能为空', trigger: 'blur' }],
                total_time: [{ required: true, type: 'number', message: '加班时长不能为空', trigger: 'change' }]
            }
        }
    },
    created() {
    ...
    methods: {
        ...
        async addSpecialist() {
            try {
                this.userFormValidate.status = '加班'
                let res = await User.addSpecialist({ addInfo: this.userFormValidate })
                if (res.data.code === 1) {
                    this.$Message.success('添加员工加班信息成功！')
                    this.modal1 = false
                    this.initData()
                }
            } catch (error) {
                console.log(error)
                this.$Message.error('添加员工加班信息失败！')
            }
        },
        async updateSpecialist(id) {
            try {
                ...
                if (res.data.code === 1) {
                    this.$Message.success('修改员工加班信息成功！')
                    this.modal1 = false
                    this.initData()
                }
            } catch (error) {
                console.log(error)
                this.$Message.error('修改员工加班信息失败！')
            }
        },
        clickAdd() {
            this.modal1 = true
        ...
        search() {
            if (this.search_model !== '') {
                this.query = { query: { user_nickname: this.search_model, status: '加班' } }
            }
            this.initData()
            this.query = {}
        },
