...
export default {
    data() {
        return {
            ...
            userRuleValidate: {
                ...
                start_time: [{ required: true, type: 'date', message: '请假日期不能为空', trigger: 'blur' }],
                total_time: [{ required: true, type: 'number', message: '请假时长不能为空', trigger: 'change' }]
            }
        }
    },
    created() {
    ...
    methods: {
        ...
        async addSpecialist() {
            try {
                this.userFormValidate.status = '请假'
                let res = await User.addSpecialist({ addInfo: this.userFormValidate })
                if (res.data.code === 1) {
                    this.$Message.success('添加员工请假信息成功！')
                    this.modal1 = false
                    this.initData()
                }
            } catch (error) {
                console.log(error)
                this.$Message.error('添加员工请假信息失败！')
            }
        },
        async updateSpecialist(id) {
            try {
                ...
                if (res.data.code === 1) {
                    this.$Message.success('修改员工请假信息成功！')
                    this.modal1 = false
                    this.initData()
                }
            } catch (error) {
                console.log(error)
                this.$Message.error('修改员工请假信息失败！')
            }
        },
        clickAdd() {
            this.modal1 = true
        ...
        search() {
            if (this.search_model !== '') {
                this.query = { query: { user_nickname: this.search_model, status: '请假' } }
            }
            this.initData()
            this.query = {}
        },
